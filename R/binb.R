...
iqss <- function(toc = FALSE,
  ...
  for (f in c("beamercolorthemeiqss.sty", "beamerfontthemeiqss.sty",
    ...
    if (!file.exists(f))
      file.copy(system.file("rmarkdown", "templates", "wakepres", "skeleton",
                            f, package="wfutemplates"),
                ".", recursive=TRUE)

  template <- system.file("rmarkdown", "templates", "wakepres",
                          "resources", "template.tex",
